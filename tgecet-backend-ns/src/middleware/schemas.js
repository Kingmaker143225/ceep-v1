...
const createApplicationSchema = z.object({
  ...
  applicationData: z.object({
    ...
    occupation_of_father: z.string().min(1),
    ssc_hall_ticket_no: z.string().min(1),
    ssc_pass_month_year: z.string().min(1),

    special_category: z.string().min(1),
    local_area_status: z.enum(["LOCAL", "NON-LOCAL"]),

    applied_subject: z.string().min(1),
    course: z.string().min(1),

    employer_name_address: z.string().min(1),
    employment_designation: z.string().min(1),

    communication_address: z.string().min(1),
    communication_district: z.string().min(1),
    communication_state: z.string().min(1),
    communication_pincode: z.string().regex(/^\d{6}$/),

    permanent_address: z.string().min(1),
    permanent_district: z.string().min(1),
    permanent_state: z.string().min(1),
    permanent_pincode: z.string().regex(/^\d{6}$/),

    photo_url: z.string().url(),
    signature_url: z.string().url()
  }),
  qualification_details: z.array(
  z.object({
    qualification_type: z.string(),
    board_university: z.string().optional(),
    hall_ticket_no: z.string().optional(),
    place_of_study: z.string().optional(),
    aggregate_percentage: z.string().optional(),
